...
def get_manual_coordinates():
    ...
    time.sleep(1.5)  # Wait 3 seconds for the user to position the mouse at the start point
    ...
    time.sleep(1.5)  # Wait 3 seconds for the user to position the mouse at the end point
...
def main():
    ...
    while True:
        ...
        if elapsed_time >= 120:
            ...
            time.sleep(15)  # Rest for 15 seconds
            start_time = time.time()  # Reset the start time after resting
        ...
        time.sleep(1.5)  # Wait a bit before the next cycle to avoid rapid loops
